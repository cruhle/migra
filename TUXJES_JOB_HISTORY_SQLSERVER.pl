#TUXJES_JOB_HISTORY_SQLSERVER.pl
...
foreach(sort keys %tabela) {
	...
	if(exists($tabela{$_}{'data_submitted'})) {
		$tmp = alteraFormato($tabela{$_}{'data_submitted'});
	} 
	printf($fp ";%s",$tmp);
	
	$tmp = '';
	if(exists($tabela{$_}{'data_started'})) {
		$tmp = alteraFormato($tabela{$_}{'data_started'});
	} 
	printf($fp ";%s",$tmp);
	...
	if(exists($tabela{$_}{'data_ended'})) {
		$tmp = alteraFormato($tabela{$_}{'data_ended'});
	} 
	printf($fp ";%s",$tmp);
