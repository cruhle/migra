...
$ficheiro =~ /(\d+)/;
$ficheiro = '/tmp/pkis/PURGED_MISSING_XX_'. $1 .'.csv';
...
$domain=0;
my %deleted_jobs;
open $fp,'<','jobs_missing_20191026.txt' or die "ERROR $!\n";
while(<$fp>) {
	chomp;
	$domain++;
	$deleted_jobs{$_}+=1;	
...
#	next if(!exists($deleted_jobs{$registo[JOB_NUMBER]}));
