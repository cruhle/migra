...
my $filesql = '/tmp/pkis/sql_start_end_job.csv';
open my $fps,'>',$filesql or die "ERROR $!\n";

my $filename = '/tmp/pkis/logs_start_end_job_sql.csv';
open my $fpo,'>',$filename or die "ERROR $!\n";
printf($fpo "%s\n",$0);
printf($fpo "DATE_TIME;DOMAIN;JOB_NAME;JOB_NUMBER;START_DATE_TIME;RUNTIME_SECONDS\n");

$start_time = DateCalcFunctions::getLocaltime();
$start_time =~ s/\-//g;

$end_time = DateCalcFunctions::getLocaltime();
$end_time =~ s/\-//g;
...
foreach(sort keys %jobs) {
	...
	$work = DateCalcFunctions::time_2_seconds(
			DateCalcFunctions::get_total_work_time(
				$jobs{$_}{'start_time'} , $start_time)
			);
...
$fp=<<"DOC";
INSERT INTO TUXJES_CRONTAB (DATE_TIME,DOMAIN,JOB_NAME,JOB_NUMBER,
START_DATE_TIME,RUNTIME_SECONDS) VALUES (
to_date('$end_time','yyyymmdd hh24:mi:ss'),'$jobs{$_}{'domain'}',
'$jobs{$_}{'job_name'}','$_',to_date('$jobs{$_}{'start_time'}','yyyymmdd hh24:mi:ss'),
$work
);
