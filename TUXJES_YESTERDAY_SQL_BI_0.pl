...
my $ficheiro = '/tmp/pkis/RAW_LOG_FILE_'. $m .'_SQL_0.csv';
open my $fpo,'>', $ficheiro or die "ERROR $!\n";
printf($fpo "%s\n",$0);
printf($fpo "DATE_TIME;DOMAIN;QTD;RUNTIME_SECONDS\n");

my ($domain, $outLine) = ('', '');
foreach(sort keys %outTimes) {

($data, $domain) = separaKEY($_);

$outLine=<<"DOC";
INSERT INTO TUXJES_DAY_HOUR (date_time, domain, jobs_run, jobs_runtime)	
  VALUES(to_date('$data','yyyymmdd hh24:mi:ss'),'$domain',$outTimes{$_}{'qtd'},$outTimes{$_}{'runtime'});

DOC

printf($fpo "%s",$outLine);		
