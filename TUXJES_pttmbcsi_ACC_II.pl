...
while(<$fp>) {
	...
	$jobs_to_run{$ins[JOB_NUMBER]}+=1;
	
	#$key = $ins[JOB_NAME] .';'.$ins[STEP_NAME];
	#$key = $ins[JOB_NUMBER].';'.$ins[STEP_NAME] .';'.$ins[JOB_NAME];
