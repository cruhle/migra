...
$ficheiro =~ /(\d+)/;
$ficheiro = '/tmp/pkis/RAW_LOG_FILE_XX_'. $m .'.csv';

open my $fp,'<',$parm or die "ERROR $!\n";
open my $fpo,'>', $ficheiro or die "ERROR $!\n";

printf($fpo "%s\n",$0);
printf($fpo "SERVER;DATE_TIME;DOMAIN;JOB_NUMBER;JOB_NAME;STEP_NAME;STEP_NUMBER;STEP_START_TIME;STEP_END_TIME;STEP_RUNTIME_SECONDS;RETURN_CODE\n");
