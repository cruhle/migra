...
printf("JOB_NAME;EXECS;TOTAL_SEGUNDOS;MEDIA_SEGUNDOS;TEMPO_MEDiO;TEMPO_TOTAL\n");
for $key (sort keys %dados_bi) {
	$total_execs += $dados_bi{$key}{'contador'};
	$total_tempo += $dados_bi{$key}{'tempo'};
	printf("%s;%05d;%05d;%04d;%s;%s\n",
		$key,
...
printf("%s;%05d;%05d;%s;%s;%s\n",
	"TOTAIS",
	$total_execs, 
	$total_tempo,
	"","",
	seconds_2_time($total_tempo)
);
