...
chomp(my @lista = `ls -t1 \$JESROOT/jessyslog/jessys.log.*`);
...
foreach my $linhas(@lista) {
	...
	while(<$fp>) {
		...
		$work = valida_tempos(time_2_seconds(substr($ins[START_TIME],1)), 
							  time_2_seconds(substr($ins[END_TIME],1)));
							  			
		if(exists($hash{$key})) {
			$hash{$key}{'steps'} .= ';' . $ins[STEP_NAME] . ';' . $ins[RETURN_CODE] . ';' . seconds_2_time($work);		
			$hash{$key}{'work'}  += $work;
		} else {
			if($contador>19) { last; }
			$contador++;
			$hash{$key} = {		
				'steps' => $ins[STEP_NAME] . ';' . $ins[RETURN_CODE] . ';' . seconds_2_time($work),
				'work' => $work
			};
		}
