#TUXJES_YESTERDAY_POOL_POWERBI.pl
...
while(<DATA>) {
	...
	while(<$fp>) {
		...
		#passa hh:mm:ss para hh:mm:00 por causa da chave (key)
		if(!exists($data_job{$registo[JOB_NUMBER]})) {
			$data_job{$registo[JOB_NUMBER]} = substr($registo[DATA_HORA],0,15) . '00';
		}
...
my $ficheiro = '/tmp/pkis/RAW_LOG_FILE_POWER_BI.csv';
...
/M_I_G_R_A/AT/jes_sys_log/rp/2019/jessys.log.100619
/M_I_G_R_A/AT/jes_sys_log/fo/2019/jessys.log.100619
/M_I_G_R_A/AT/jes_sys_log/pr/2019/jessys.log.100619
/M_I_G_R_A/AT/jes_sys_log/misc/2019/jessys.log.100619